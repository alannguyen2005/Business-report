...
def build_deck():
    ...
    # Đường dẫn tương đối từ thư mục ai_analyst/ — không hardcode ổ đĩa
    out_path = str(Path(__file__).resolve().parents[1]
                   / "data" / "reports" / "revenue" / "techworld_data" / "slidedeck.pptx")
    prs.save(out_path)
    print(f"\nSaved: {out_path}")
