...
**Setting `PYTHONPATH`:** helper imports resolve from the `ai_analyst/` directory, so set it
relative to where you are — never hardcode an absolute path (it breaks on CI runners and on
any other machine).

```bash
# Linux / macOS / GitHub Actions — run from repo root
PYTHONPATH=ai_analyst python ai_analyst/scripts/build_pptx_v3.py   --stem {stem} --output data/reports/{stem}/slidedeck.pptx
```

```powershell
# Windows PowerShell — run from repo root
$env:PYTHONPATH = "ai_analyst"
python ai_analyst/scripts/build_pptx_v3.py --stem {stem} --output data/reports/{stem}/slidedeck.pptx
```
