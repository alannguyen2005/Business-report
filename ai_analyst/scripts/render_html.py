...
def render_html(stem: str, output_path: str | None = None,
                pipeline_dir: str | None = None) -> None:
    # pipeline_dir lets artifacts live outside ai_analyst/ (e.g. repo root).
    # Templates still resolve from BASE — only the data is relocatable.
    pipeline = Path(pipeline_dir) if pipeline_dir else BASE / f"data/pipeline/{stem}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pipeline-dir", default=None,
                        help="Directory holding report_context.json + chart_specs.json "
                             "(default: ai_analyst/data/pipeline/{stem})")
    args = parser.parse_args()
    render_html(args.stem, args.output, args.pipeline_dir)
